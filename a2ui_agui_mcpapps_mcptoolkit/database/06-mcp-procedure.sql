...
CREATE OR REPLACE PROCEDURE create_customer_follow_up_mcp (
...
BEGIN
    IF p_action_id < 1 THEN
        RAISE_APPLICATION_ERROR(-20004, 'Action ID must be positive');
    END IF;
    ...
    INSERT INTO customer_actions (
        action_id, customer_id, action_type, action_notes, requested_by, action_status
    ) VALUES (
        p_action_id, p_customer_id, p_action_type, p_action_notes, p_requested_by, 'APPROVED'
    );
    ...
    -- The toolkit commits only after this complete tool statement succeeds.
EXCEPTION
