...
if [[ -f "$ENV_HELPER" ]]; then
  ...
  load_env_defaults "$ENV_FILE" \
    ...
    SELECT_AI_PROFILE \
    DBMS_CLOUD_AI_PROFILE \
    SELECT_AI_PROMPT_SCOPE \
    INVENTORY_SYSTEM_SELECT_AI_SCOPE \
    INVENTORY_SYSTEM_ALLOW_LOCAL_SELECT_AI_FALLBACK \
    ORACLE_AI_DATABASE_AGENT_URL \
    ORACLE_AI_DATABASE_AGENT_AUTHORIZATION_URL \
    ORACLE_AI_DATABASE_AGENT_TOKEN_URL \
    ORACLE_AI_DATABASE_AGENT_CLIENT_ID \
    ORACLE_AI_DATABASE_AGENT_CLIENT_SECRET \
    ORACLE_AI_DATABASE_AGENT_REFRESH_TOKEN \
    ORACLE_AI_DATABASE_AGENT_BEARER_TOKEN \
    ORACLE_AI_DATABASE_AGENT_AUTHORIZATION_HEADER \
    ORACLE_AI_DATABASE_AGENT_TIMEOUT_SECONDS \
    ORACLE_AI_DATABASE_AGENT_TASK_POLL_ATTEMPTS \
    ORACLE_AI_DATABASE_AGENT_TASK_POLL_INTERVAL_MILLIS \
    MODEL_NAME \
